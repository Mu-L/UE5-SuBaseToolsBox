...
// =====================================================================
// 拖拽操作：带着一批 FCopyItem 在面板间飞
// =====================================================================

// 一个小巧的"拖放接收框"：SBorder 本身不暴露 OnDrop 事件，
// 这里继承它，把虚函数 OnDrop / OnDragOver 接到我们自己写的回调上。
...
void SVariableCopier::Construct(const FArguments& InArgs)
{
		...
		[
			...
			[
				SNew(STextBlock).Text(FText::FromString(
					FString::Printf(TEXT("蓝图 %d（拖出去=复制/剪切到对面）"), PanelIdx + 1)))
			]
			...
			[
				SNew(STextBlock)
				.Text(LOCTEXT("Tip", "Ctrl 多选后拖到对面面板。普通/Shift=复制，Alt=剪切。列表按类型分在各自折叠区里。"))
				.ColorAndOpacity(FSlateColor::UseSubduedForeground())
				.AutoWrapText(true)
			]
	...
	AppendLog(TEXT("就绪：左右各选一个蓝图 -> 在一边 Ctrl 多选 -> 拖到对面。Alt 拖=剪切，Shift/普通拖=复制。点底部「确定」才真正写入。"));
}
